...
export const storageService = {
  ...
  importData: (jsonData: string): User | null => {
      try {
          ...
          // Check if it is a full backup (contains user and progress)
          if (data.user && data.progress) {
              const rawUser = data.user;
              
              // Normalize name: Prioritize username, then userName, then name. 
              // If none exist, use empty string to let user fill it in AuthScreen.
              ...
              // 1. Register User (only if name exists, otherwise AuthScreen handles registration on login)
              if (name) {
                storageService.register(user);
                // 2. Save Progress
                storageService.saveProgress(user.email, progress);
                
                // 3. Save History if exists
  ...
  // Initialize or load existing bot scores
  getBots: (): BotData[] => {
  ...
  // Simulate activity by increasing scores slightly
  updateBotScores: () => {
      const bots = storageService.getBots();
      const updatedBots = bots.map(bot => {
          // 30% chance to gain points on screen change
          if (Math.random() > 0.7) {
  ...
  getLeaderboard: (currentUser: User, currentScore: number): LeaderboardEntry[] => {
      ...
      // Logic: Get top 10. If user is not in top 10, append them as 11th.
