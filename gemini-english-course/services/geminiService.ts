...
const callAIWithFailover = async (options: AIServiceOptions): Promise<string> => {
  ...
  // Define an array of fallback keys here. The system will try each one sequentially if the previous ones fail.
  const FALLBACK_KEYS: string[] = [
    BACKUP_GEMINI_KEY,
    // Add more fallback keys here as needed:
    // "sk-or-v1-...",
    // "AIzaSy...",
  ];

  for (const fallbackKey of FALLBACK_KEYS) {
      if (fallbackKey && !attempts.some(a => a.key === fallbackKey)) {
          attempts.push({
              key: fallbackKey,
              isOpenRouter: fallbackKey.startsWith("sk-or-")
          });
      }
  }
