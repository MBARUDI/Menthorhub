...
// FIX: Manually define Speech Recognition API types to resolve TypeScript errors.
...
const ConversationComponent: React.FC<ConversationComponentProps> = ({ userName, onSpeak, speakingText, targetLanguage, onUserTurn }) => {
  ...
  // Map TargetLanguage to BCP-47 language tags for Speech Recognition
  ...
  const sendMessage = useCallback(async (messageText: string) => {
    ...
    // Award points for the user's answer
    if (onUserTurn) {
        onUserTurn();
    }
  ...
  useEffect(() => {
    ...
    // CRITICAL: Force continuous to FALSE to prevent echo/duplication on mobile.
    // The user will speak one sentence, and it will auto-stop or can be manually toggled off.
    ...
    const handleResult = (event: SpeechRecognitionEvent) => {
      if (event.results.length > 0) {
        // CRITICAL: Just take the first result since continuous=false.
        // Do not loop through results to avoid appending errors.
    ...
    const handleEnd = () => {
      ...
      // Automatically send message when silence is detected (standard mobile behavior)
