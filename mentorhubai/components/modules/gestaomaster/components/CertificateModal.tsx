...
const CertificateModal: React.FC<CertificateModalProps> = ({ isOpen, onClose, lessons }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-955/95 backdrop-blur-md p-4 animate-fadeIn text-slate-900">
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl overflow-hidden relative flex flex-col h-full max-h-[90vh] md:max-h-[85vh]">
        ...
        {viewState === 'intro' && (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-slate-50">
            ...
            <h2 className="text-3xl font-bold text-slate-800 mb-4 font-sans">Prova Final de Certificação</h2>
            <div className="max-w-lg bg-white p-6 rounded-xl border border-slate-200 shadow-sm mb-8 text-left space-y-3 font-sans">
            ...
            <button 
              onClick={startExam}
              className="px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg rounded-xl shadow-lg hover:shadow-blue-500/30 transition-all flex items-center gap-3 active:scale-95 font-sans"
            >
              Iniciar Prova Agora
            </button>
          </div>
        ...
        {viewState === 'exam' && examData && (
          <div className="flex-1 flex flex-col bg-slate-50 overflow-hidden">
             ...
             <div className="flex-1 overflow-y-auto p-6 md:p-8 w-full max-w-4xl mx-auto custom-scrollbar">
                               ...
                               return (
                                  <button
                                     ...
                                     className={`w-full text-left p-3 rounded-lg border transition-all flex items-center gap-3
                                        ${isSelected 
                                           ? 'bg-blue-50 border-blue-500 text-blue-800 ring-1 ring-blue-500 font-medium' 
        ...
        {viewState === 'result' && examData && (
           <div className="flex-1 flex flex-col bg-slate-50 overflow-hidden">
              <div className="bg-white border-b border-slate-200 p-6 text-center shadow-sm z-10 shrink-0">
                 <div className="mb-2">
                    {passed ? (
                 ...
                 <div className="mt-6 flex justify-center gap-4">
                    {!passed && (
                       <button onClick={handleRetry} className="px-6 py-2.5 bg-slate-200 hover:bg-slate-300 text-slate-800 font-bold rounded-lg transition-colors flex items-center gap-2 active:scale-95">
                          <RefreshCw size={18} /> Tentar Novamente
                       </button>
                    )}
                    {passed && (
                       <button onClick={proceedToCertificate} className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow-lg transition-colors flex items-center gap-2 animate-bounce active:scale-95">
                       ...
                       return (
                          <div key={q.id} className={`p-5 rounded-xl border-l-4 shadow-sm bg-white ${isCorrect ? 'border-l-green-500' : 'border-l-red-500'}`}>
                             <div className="flex items-start justify-between gap-4 mb-3">
                                ...
                                {isCorrect 
                                   ? <CheckCircle2 className="text-green-500 shrink-0 animate-scaleUp" size={20} />
                                   : <XCircle className="text-red-500 shrink-0 animate-scaleUp" size={20} />
                                }
                             </div>
        ...
        {viewState === 'form' && (
          <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 text-center bg-slate-50 w-full animate-fadeIn">
            <div className="w-full max-w-md">
                ...
                <p className="text-slate-600 mb-8 text-sm">Preencha seus dados para a emissão oficial do diploma.</p>
                
                <form onSubmit={handleGenerateCertificate} className="space-y-4 text-left font-sans">
                   <div>
                   ...
                   <button 
                      type="submit" 
                      className="w-full py-4 bg-slate-900 hover:bg-slate-800 text-white font-bold text-lg rounded-xl shadow-lg mt-4 transition-all flex items-center justify-center gap-2 active:scale-95 shadow-slate-900/10"
                   >
                        ...
                        <div className="flex justify-center mb-6">
                          <div className="flex items-center gap-5">
                            ...
                            <div className="text-left">
                              <h1 className="text-3xl font-bold uppercase tracking-widest leading-none text-slate-900 font-sans">GestãoMaster AI</h1>
                              <p className="text-xs tracking-[0.4em] uppercase text-blue-800 font-bold mt-1 font-sans">MBA Executivo Digital</p>
                            </div>
                          </div>
                        </div>
